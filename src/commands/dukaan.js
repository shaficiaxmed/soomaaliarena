...
module.exports = {
    ...
    execute(message) {
        db.get(`SELECT * FROM users WHERE user_id = ?`, [message.author.id], (err, row) => {
            if (err) return message.channel.send("❌ Cilad ayaa dhacday.");
            ...
            message.channel.send({ embeds: [embed], components: [rowButtons] }).then(sentMessage => {
                ...
                collector.on('collect', async interaction => {
                    db.get(`SELECT * FROM users WHERE user_id = ?`, [message.author.id], async (err, userRow) => {
                        if (!userRow) return interaction.reply({ content: "❌ Xogtaada lama helin.", ephemeral: true });

                        // 1. SHIELD LOGIC
                        if (interaction.customId === 'buy_shield') {
                            const isShieldActive = userRow.shield_expiry && new Date(userRow.shield_expiry) > new Date();
                            ...
                            if (diamonds < 14) return interaction.reply({ content: `❌ Diamonds-kaagu kuguma filna! Waxaad haysataa **${diamonds} 💎**.`, ephemeral: true });

                            const newExpiry = new Date(Date.now() + 6 * 60 * 60 * 1000).toISOString();
                            db.run(`UPDATE users SET diamonds = diamonds - 14, shield_expiry = ? WHERE user_id = ?`, [newExpiry, message.author.id], () => {
                        ...
                        // 2. HP LOGIC
                            ...
                            if (userRow.hp >= 50) return interaction.reply({ content: `💖 Caafimaadkaagu wuu fiican yahay (**${userRow.hp}%**). Uma baahnid inaad HP iibsato.`, ephemeral: true });
                            ...
                            db.run(`UPDATE users SET wallet = wallet - 200, hp = 100 WHERE user_id = ?`, [message.author.id], () => {
                        ...
                        // 3. ENERGY LOGIC
                            ...
                            if (userRow.energy >= 50) return interaction.reply({ content: `⚡ Tamartaadu way fiican tahay (**${userRow.energy}%**). Uma baahnid inaad Energy iibsato.`, ephemeral: true });
                            ...
                            db.run(`UPDATE users SET wallet = wallet - 100, energy = 100 WHERE user_id = ?`, [message.author.id], () => {
