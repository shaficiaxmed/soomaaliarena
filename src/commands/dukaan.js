...
module.exports = {
    ...
    execute(message) {
        db.get(`SELECT * FROM users WHERE user_id = ?`, [message.author.id], (err, row) => {
            ...
            // Shaqada oo furaysa dukaanka marka la hubiyo ama la diiwaan geliyo user-ka
            const proceedShop = () => {
            ...
            // Haddii uusan user-ku jirin, diiwaan geli marka hore kadibna fur dukaanka
            if (!row) {
                db.run(`INSERT INTO users (user_id, wallet, diamonds, hp, energy) VALUES (?, 1000, 10, 100, 100)`, [message.author.id], (err) => {
